...
public class FluentTaskChecklistsApiTests
{
    ...
    public async Task FluentTaskChecklistsApi_CreateChecklist_ShouldBuildCorrectRequestAndCallService()
    {
        ...
        var expectedResponse = new CreateChecklistResponse { Checklist = new Client.Models.Entities.Checklists.Checklist("checklistId", "testTaskId", checklistName, 0, 0, 0, null) };

        var mockTaskChecklistsService = new Mock<ITaskChecklistsService>();
        mockTaskChecklistsService.Setup(x => x.CreateChecklistAsync(
            It.IsAny<string>(),
            It.IsAny<Client.Models.RequestModels.Checklists.CreateChecklistRequest>(),
            It.IsAny<bool?>(),
            It.IsAny<string?>(),
            It.IsAny<CancellationToken>()))
            .ReturnsAsync(expectedResponse);

        var fluentTaskChecklistsApi = new TaskChecklistsFluentApi(mockTaskChecklistsService.Object);
        ...
        mockTaskChecklistsService.Verify(x => x.CreateChecklistAsync(
            taskId,
            It.Is<Client.Models.RequestModels.Checklists.CreateChecklistRequest>(req =>
