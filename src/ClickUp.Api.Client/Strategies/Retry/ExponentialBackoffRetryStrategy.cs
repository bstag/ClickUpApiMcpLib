...
namespace ClickUp.Api.Client.Strategies.Retry
{
    ...
    public class ExponentialBackoffRetryStrategy : IRetryStrategy
    {
        ...
        public ExponentialBackoffRetryStrategy(
            ILogger<ExponentialBackoffRetryStrategy>? logger = null,
            int maxRetries = 3,
        ...
        public TimeSpan CalculateDelay(int attemptNumber, Exception? exception = null, HttpResponseMessage? response = null)
        {
            ...
            if (response?.Headers.RetryAfter != null)
            {
                ...
                if (retryAfter.Delta.HasValue)
                {
                    ...
                    return headerDelay > _maxDelay ? _maxDelay : headerDelay;
                }
            ...
            var exponentialDelay = TimeSpan.FromMilliseconds(
                _baseDelay.TotalMilliseconds * Math.Pow(_backoffMultiplier, attemptNumber - 1));
            ...
            if (_useJitter)
            {
                ...
                exponentialDelay = TimeSpan.FromMilliseconds(Math.Max(0, exponentialDelay.TotalMilliseconds + jitter));
            }

            _logger?.LogDebug("Calculated delay for attempt {Attempt}: {Delay}", attemptNumber, exponentialDelay);
